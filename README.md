...
>
> 当前发布版 **v260910** · 对齐 S4A21 `c58cd2b`、数据库契约 **schema v26** · MIT License
...
## v260910 S4A21 编码与服务端契约更新

发布下载：[v260910 Release](https://github.com/Liuxiny/86jp_DfoGmTool/releases/tag/v260910)。`main` 与 `v260910` 分支同步本次源码和说明。

| 附件 | 用途 |
|------|------|
| `dfo-gm-tool_260910_win-x64.zip` | Windows x64 GM 工具，解压整个目录后运行 |
| `dfo-gm-tool_260910_linux-x64.tar.gz` | Linux x64 GM 工具，解压整个目录后运行 |
| `S4A21ClientPatch_260910_GBK.zip` | S4A21_CN 客户端 GBK 适配补丁，包含 `az.dll` 和 `GameGaurd.dll` |

**客户端乱码修复**：使用 GBK 服务端时，关闭 S4A21_CN 客户端，备份客户端目录原有的 `az.dll` 和 `GameGaurd.dll`，再将补丁压缩包内两个同名 DLL 覆盖到客户端目录并重新启动。补丁来自 [S4A21ClientPatch](https://gitgud.io/rewio/S4A21ClientPatch)，用于移除旧客户端强制 UTF-8 的编码行为，适配 GBK 服务端，修复由编码不匹配造成的角色名、邮件、活动文案乱码。注意文件名为 `GameGaurd.dll`。更换 DLL 不会自动修复数据库中已损坏的文字；旧名字节按下方加载前编码检查选择正常文字并转换。

- **角色属性 → 等级与经验**：副职业和决斗等级使用下拉选择。副职业界面只提交所选等级，后端从当前 PVF `.exj` 读取经验阈值并写入最低经验；分解师等级沿用服务端 `GetExpertJobLevel`，其他职业沿用配方配置的等级上限。胜点修改移至角色货币页面，金币／复活币／胜点按固定槽位显示名称，零余额缺行也可编辑。
- **副职业切换事务**：依据 `[connect quest list]` 完成所选任务链，依据 `[clear quests]` 清除其余职业任务的进行中／完成记录，并验证 PVF `[collision quest]` 确实能隐藏其他职业任务。同步职业技能、初始设备状态和自动配方；同职业改等级保留手动学习的配方，切换职业清空旧配方。选择“无副职业”会释放全部职业任务，允许正常重新选择。此操作不补发任务物品奖励，不改变分解机的独立升级规则。
- **分解机等级**：仅已保存为分解师的角色显示副职业卡片内的分解机等级下拉框。可选等级来自 `disjointer.exj` 的升级／修理配置，并受副职业等级、`expertjob.etc` 角色等级要求约束（当前 PVF 1–11 级）。单独修改设备等级，变更时恢复目标等级满耐久，不扣金币；同级保存保留当前耐久。修改前确认离线，后端核对职业、经验、角色等级、设备等级及耐久的状态指纹，事务写入，不修改技能栏等级；其他副职业无独立技能升级入口。未保存的职业选择不提前显示设备控件。
- **决斗数据**：等级下拉项按当前 PVF `etc/pvp_ref.etc` 的 `[pvp grade point]` 读取合法编号，中文名称对照 S4A21_CN `dstr.dat` FILE/102 的 CNUser.cpp 字串 1207–1227：入门、青铜／白银／黄金／钻石／泰拉石各 1–4 星。界面不显示编码，不提供未核实编码关系的排名称号；修改 `characters.pvp_grade` 时保留独立的 `pvp_rating_grade`。胜点使用 A21 主背包虚拟槽 2 的 99B ItemCore，并记库存审计，前后端均限制手动设置为 0–999999，保留已有 Core 的其他字节。副职业和决斗卡片左右并列，位于转职／觉醒下方。等级与胜点独立保存，修改须角色退出或切换到其他角色后执行，重新登录验证客户端显示。
- **任务提交道具**：“标记可交”和每日任务“接取并可交”读取 PVF 的 seeking 物品要求，合并重复物品编号，扣除主背包已有数量后补足。金币、晶块、灵魂等固定资产使用服务端对应存储；普通道具按实际开放槽位堆叠／发放，空间不足时回滚该次背包发放并改用系统邮件。未领取的任务补发邮件计入待领取数量，重复点击不会重复发放；道具、邮件和任务状态在同一事务中写入，失败全部回滚。操作后提示在游戏中重新选择角色；邮件附件须腾出背包空间并领取后才能交任务。强制完成仍不发道具或奖励。

- **GBK 名字节**：角色列表、复制、改名、重名检测、异常物品列表统一按服务端 `ClientTextEncoding`（936）读写。复制与改名共用 2–12 个 GBK 字节限制，最多 6 个中文（全角）或 12 个字母、数字（半角），混合时全角占 2 字节；不能用 GBK 表示的字符直接拒绝，不替换成问号。左侧角色名旁点击编辑符号，确认角色不在线后输入新名字；后端验证旧名字与重名，名字字段与双向好友关系同一事务修改，失败全部回滚。日志、配置、密码哈希、邮件正文和好友 TEXT 不转为 GBK。
- **结构兼容门禁**：只检查实际必需表、列和 99 字节 ItemCore，不强制 schema 版本、baseline 或版本元数据一致性；版本信息仅用于诊断。
- **加载前编码检查**：结构兼容后、PVF 和 GM 初始化前检查 `characters.name/name_bytes`、`character_creatures.creature_text`、邮件附件 `detail_json` 中宠物 `NameBytes`。存在旧 UTF-8、双编码歧义或非法字节时释放数据源，在数据源窗口打开“旧数据库 UTF-8 → GBK 编码转换”；处理完成后重新检查并加载。A12 结构迁移仍使用独立入口。
- **编码转换**：先停服并预览，逐项选择显示正常、符合原名的文字：UTF-8 正常就选 UTF-8，工具自动转为 GBK；GBK 正常就选 GBK 并保留原内容。两项都乱码时先取消并核对原名。执行弹窗只需点击确认，无需输入内容。使用 SQLite 备份 API 在数据库旁生成 `.before-gbk-时间-唯一编号.db`（包含 WAL 数据），校验预览指纹，在写事务内处理，冲突或非法字符回滚；邮件标题、正文、活动文案等 Unicode TEXT 不转码，不改 schema 版本。新增 `gm_client_text_gbk` 表记录已确认字段的内容指纹，避免重复转换，内容改变后会重新检查。
- **备份 v4**：新导出带 `clientTextCodePage=936`；仍接受旧 A21 v3，恢复时转换角色名、宠物名及邮件附件中的宠物名字节。恢复使用备份的私有副本，失败和重试不会重复转码。v1/v2 仍拒绝。好友关系按所属角色的 Unicode 名称导出，恢复自身的好友列表；永久删除角色清理双向关系。
- **角色槽位**：备份恢复、A12 迁移和永久删除按新版 schema v12 规则压缩活跃角色槽位空洞，保持原有排列顺序。
- **技能重建**：固定等级技能读取 PVF 定义，多值最高等级按转职方向索引，并应用觉醒方向上限；转职、觉醒和 SP 账本使用相同上下文。
- **契约同步**：schema v26、PvfLib、装备类型、任务契约和 `ClientTextEncoding` 从最新服务端同步，源提交及哈希记录在 `server-contract-manifest.json`。A12 一次性迁移同时转换旧名字节，仍在独立文件完成并验证后替换。

客户端选角、中文名显示和邮件领取仍需实机验证；本次自动化验证覆盖数据库与 GM 业务，不代表客户端联调已完成。
...
DfoGmTool.exe --selftest-inventory-a21
DfoGmTool.exe --selftest-a12-to-a21-migration
```
